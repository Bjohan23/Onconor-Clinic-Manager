...
const InvoicesPage = () => {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const response = await invoiceService.getInvoicesWithPagination(
        currentPage,
        itemsPerPage,
        filters
      );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm" style={{ color: colors.text.secondary }}>
            Administra las facturas de los pacientes
            {totalItems > 0 && (
              <span className="ml-2 font-medium">
                ({totalItems} factura{totalItems !== 1 ? 's' : ''} registrada{totalItems !== 1 ? 's' : ''})
              </span>
            )}
          </p>
        </div>
      ...
      <Card>
        {loading ? (
          <LoadingSpinner size="lg" text="Cargando facturas..." center />
        ) : (
          <>
            <Table
              columns={columns}
              ...
              emptyMessage={
                hasFilters
                  ? "No se encontraron facturas con los filtros aplicados. Intenta ajustar tus criterios de búsqueda."
                  : "No hay facturas registradas aún. Haz clic en 'Nueva Factura' para agregar la primera."
              }
              hover
            />
            {/* Paginación */}
            {totalPages > 1 && (
              <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t pt-4"
                ...
                <div className="text-sm" style={{ color: colors.text.secondary }}>
                  Mostrando {((currentPage - 1) * itemsPerPage) + 1} - {Math.min(currentPage * itemsPerPage, totalItems)} de {totalItems} facturas
                </div>
